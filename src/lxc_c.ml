...
let lxc_get_wait_states =
  Foreign.foreign "lxc_get_wait_states"
    (ptr string @-> returning int)

let lxc_get_global_config_item =
  Foreign.foreign "lxc_get_global_item"
    (string @-> returning string)

let lxc_get_version =
  Foreign.foreign "lxc_get_version"
    (void @-> returning string)

let list_defined_containers =
  Foreign.foreign "list_defined_containers"
    (string @-> ptr (ptr (ptr char)) @-> ptr (ptr (ptr Stubs.lxc_container)) @-> returning int)
...
let lxc_log_init =
  Foreign.foreign "lxc_log_init"
    (ptr Stubs.Lxc_log.t @-> returning int)

let lxc_log_close =
  Foreign.foreign "lxc_log_close"
    (void @-> returning void)

let lxc_config_item_is_supported =
  Foreign.foreign "lxc_config_item_is_supported"
    (string @-> returning bool)

let lxc_has_api_extension =
  Foreign.foreign "lxc_has_api_extension"
    (string @-> returning bool)
